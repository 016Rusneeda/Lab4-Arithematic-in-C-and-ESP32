...
void app_main(void)
{
    ...
    int cookies_per_personn = total_cookies / number_of_friends;
    int remaining_cookiess = total_cookies % number_of_friends;
  ...
  /*  if (remaining_cookiess == 0) {
        ESP_LOGI(TAG, "✅ หารลงตัว! ทุกคนได้เท่ากัน");
    } else {
        ESP_LOGI(TAG, "⚠️ หารไม่ลงตัว! เหลือ %d ชิ้น", remaining_cookiess);
    }
    ESP_LOGI(TAG, "");
    ESP_LOGI(TAG, "");
